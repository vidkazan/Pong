...
export default function ChatViewBottomLine(props: ChatViewProps) {
    ...
    const sendMessage = () => {
        ...
        if (inputLine.current.value !== "" && props.parentProps.mainProps.mainProps.webSocket) {
            // @ts-ignore
            let str :String = inputLine.current.value;
            while(str.length){
                let msg: String = "";
                if(str.length > 500) {
                    msg = str.slice(0, 499);
                    str = str.slice(500,str.length-1);
                }
                else {
                    msg = str;
                    str = "";
                }
                props.parentProps.mainProps.mainProps.webSocket.emit('message', {data: msg })
                // console.log(props);
                props.parentProps.mainProps.mainProps.webSocket.emit('message:post',  {userId : props.parentProps.mainProps.mainProps.userId ,userName: props.parentProps.mainProps.mainProps.userName, text: msg , ChatId: props.parentProps.onChatId})
            }
